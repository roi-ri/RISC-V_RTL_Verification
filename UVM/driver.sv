...
$root.top.dut.MEM[i] = 32'h00000013; // Esperar a ver como lo va a llamar luis, pero esto escribe NOP
    ...
    task create_write_instr(my_sequence_item my_sequence_item_obj); 
        ...
        case (my_sequence_item_obj.instr_type) 
            ...
            case (my_sequence_item_obj.i_arith_instr) 
                ...
                SLTIU:begin
                    ...
                    //instruccion
                    instr_wrd = i_arithmetic_instr; 
                    $display("Instruccion SLTIU creada: 0x%08h", instr_wrd);
                end
                XORI: begin 
                    ...
                    //instruccion
                    instr_wrd = i_arithmetic_instr; 
                    $display("Instruccion XORI creada: 0x%08h", instr_wrd);
                end 
                ORI: begin 
                    ...
                    //instruccion
                    instr_wrd = i_arithmetic_instr; 
                    $display("Instruccion ORI creada: 0x%08h", instr_wrd);
                end 
    ANDI: begin
    ...
    //instruccion
                    instr_wrd = i_arithmetic_instr; 
                    $display("Instruccion ANDI creada: 0x%08h", instr_wrd);
                end 
                default: begin 
            ...
            B_TYPE: begin
                ...
                case(my_sequence_item_obj.b_instr)
                    BEQ: begin
                        ...
                        $display("Instruccion BEQ creada: 0x%08h", instr_wrd);
                    end
                    BNE: begin
                        b_branch_instr.funct3 = 3'b001;
                        instr_wrd = b_branch_instr;
                        $display("Instruccion BNE creada: 0x%08h", instr_wrd);
                    end
                    BLT: begin
                        b_branch_instr.funct3 = 3'b100;
                        instr_wrd = b_branch_instr;
                        $display("Instruccion BLT creada: 0x%08h", instr_wrd);
                    end
                    BGE: begin
                        b_branch_instr.funct3 = 3'b101;
                        instr_wrd = b_branch_instr;
                        $display("Instruccion BGE creada: 0x%08h", instr_wrd);
                    end
                    BLTU: begin
                        b_branch_instr.funct3 = 3'b110;
                        instr_wrd = b_branch_instr;
                        $display("Instruccion BLTU creada: 0x%08h", instr_wrd);
                    end
                    BGEU: begin
                        b_branch_instr.funct3 = 3'b111;
                        instr_wrd = b_branch_instr;
                        $display("Instruccion BGEU creada: 0x%08h", instr_wrd);
                    end
                    default: begin
                        instr_wrd = 32'h00000013; // NOP: addi, x0, x0, 0
                        $display("Error en creacion de la instruccion");
            ...
            U_TYPE: begin
                ...
                case(my_sequence_item_obj.u_instr)
                    LUI: begin
                        ...
                        instr_wrd = u_instr;
                        $display("Instruccion LUI creada: 0x%08h", instr_wrd);
                    end
                    AUIPC: begin
                        u_instr.opcode = 7'b0010111;
                        instr_wrd = u_instr;
                        $display("Instruccion AUIPC creada: 0x%08h", instr_wrd);
                    end
                    default: begin
                        instr_wrd = 32'h00000013; // NOP: addi, x0, x0, 0
                        $display("Error en creacion de la instruccion");
            ...
            J_TYPE: begin
                ...
                case(my_sequence_item_obj.j_instr)
                    JAL: begin
                        instr_wrd = j_instr;
                        $display("Instruccion JAL creada: 0x%08h", instr_wrd);
                    end
                    default: begin
                        instr_wrd = 32'h00000013; // NOP: addi, x0, x0, 0
                        $display("Error en creacion de la instruccion");
        ...
        endcase 
        $root.top.dut.MEM[my_sequence_item_obj.addr] = instr_wrd;
    endtask
